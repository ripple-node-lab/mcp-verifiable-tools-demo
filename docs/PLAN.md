...
│   │       ├── cbor.ts           # CBOR（RFC 8949）エンコード / デコード（Phase 3-a）
│   │       ├── cose.ts           # COSE_Sign1（RFC 9052）エンコード / デコード（Phase 3-a）
│   │       └── index.ts
...
│   │       ├── tee-nitro.ts      # tee-nitro-v1（COSE_Sign1 attestation を生成、モック fixtures。Phase 3-a）
│   │       └── index.ts
...
│   │       ├── tee-nitro.ts      # tee-nitro-v1 検証（chain / PCR / user_data / nonce / freshness。Phase 3-a）
│   │       └── index.ts
...
│   │       ├── prover.ts         # SidecarProver（binding フィールドの echo を強制検査）
│   │       ├── verifier.ts       # SidecarVerifier
│   │       └── health.ts
│   ├── sdk-extension/            # @modelcontextprotocol/sdk 1.30.0 上のアダプタ（Phase 4-a）
│   ├── sidecar-mock/             # 契約のリファレンス実装（demo-sig-sidecar-v1, node:http。Phase 3-a）
│   └── client/                   # 検証クライアント
│       └── src/
...
│   └── sdk-extension.test.ts     # Phase 4-a（InMemory + Streamable HTTP 両 transport）
...
| `sdk-extension.test.ts`（Phase 4-a 既定） | InMemory + Streamable HTTP 両 transport で: initialize capability 広告、demo-sig/snarkjs の検証、tasks ポーリング、deferred proof、blind call、provenance（正常 + feed 障害 -32603）、拡張なしサーバー拒否、proof 改ざん拒否 |
...
| 4-a（完了）SDK ブリッジ | `packages/sdk-extension`: 公開版 `@modelcontextprotocol/sdk@1.30.0` ピン（+ zod 4.5.4）の `Server` / `Client` 上に拡張を載せる薄いアダプタ。サーバー側は `attachVerifiableTools` が `registerCapabilities({ tools, extensions })`（`DemoServer.discoveryExtensions` で `server/discover` と同一の capability を initialize 結果に乗せる）+ 各メソッドを `setRequestHandler`（緩い zod schema）で `DemoServer.dispatch` へ委譲。`tasks/get` / `tasks/cancel` は SDK 1.30 が登録時に `capabilities.tasks`（SDK 固有の tasks 形）を要求するため `fallbackRequestHandler` 経由。クライアント側は `sdkRpc` / `createVerifiableClient` / `assertServerSupportsVerifiableTools`（initialize の `capabilities.extensions` に拡張が無ければ失敗）+ `VerifiableClientOptions.rpc` seam。`tests/sdk-extension.test.ts` が InMemory + Streamable HTTP 両 transport で 16 ケースを検証。既知の差分: initialize は SDK 側で 2025-11-25 をネゴシエート（1.x は 2026-07-28 未公開、拡張側は引き続き `_meta` の `io.modelcontextprotocol/protocolVersion` を送る）、`Mcp-Method` / `Mcp-Name` ヘッダは SDK transport では強制されない（stateless mode では 1.30.0 が post-initialize 通知を 500 で落とす既知問題があり、テストは stateful session）、tasks は SDK experimental tasks ではなく拡張側の custom method として通す。Python SDK 版は `ezkl-v1` サーバー側の第 2 参照実装として位置づける（#4 §4-4） | SDK 上で全シナリオ（proof / tasks / deferred / blind / provenance）が検証できる |
| 4-b SDK v2 移行（未着手） | `typescript-sdk` v2（2026-07-28 wire、`server/discover` 正規 API、`extensions` capability 正式サポート）が公開されたら `initialize` の protocolVersion を 2026-07-28 に上げ、`server/discover` を SDK ネイティブ実装へ載せ替え | SDK フォーク/ブランチ |
...
| SEP 提出（Reference Implementation 節更新済み） | `docs/spec/verifiable-tools.md`（英・日）の Reference Implementation 節を実装済み Phase（1〜4-a）・付録の形式プロファイル（risc0-v1 / ezkl-v1 / tee-nitro-v1 / oracle-sig-v1 / zktls-tlsn-v1）・CI と `docs/BENCHMARKS.md`（Phase 3 計測値を転記）・テストファイル対応表へのリンクで更新済み。残り: `modelcontextprotocol/modelcontextprotocol` への SEP PR 提出と `experimental-ext-*` incubation の判断は未着手 | SEP PR |
