...
- **Authorization continuity**: Execution integrity does not carry authority. In a multi-hop chain (Scenario E) a verified result proves that `f` ran on `X`, not that the hop which supplied `X` was allowed to. Policy layers MUST evaluate the two separately. The point of contact is `inputCommitment`: an authority layer that knows which arguments were approved can commit to them with the same construction (§Result binding; for `tools/call` the salt is empty, so the commitment is recomputable from the approved arguments alone) and reject a verified result whose `inputCommitment` differs, without this extension taking on authorization semantics. Keying both checks on `circuitHash` and tool name as well ensures a verified-but-unauthorized result is rejected by the authorization check, not accepted because it verified.
