...
> 注：本稿は `verifiable-tools.md`（英語版・SEP 形式）の日本語訳・解説版です。規範となるのは英語版であり、MCP リポジトリへの提出も英語版を使用します。両版の内容が食い違う場合は英語版が優先します。
...
| `blindPublicKeys` | `object` | サーバー側：`encryptionScheme` の値から、その方式がサーバー保持の受信者鍵へ暗号化する列挙済み方式ごとの base64url 公開鍵（`hpke-v1` では生 X25519 鍵）へのマップ。少なくとも 1 つ該当する方式が列挙されている場合、`blindExecution: true` では必須である。クライアント鍵方式（クライアントが復号鍵を保持する予約済みの `fhe-tfhe-v1`）にはエントリを含めてはならない |
...
| `replyPublicKey` | `string` | ブラインド呼び出し用：ツール出力も機密にする必要がある場合に、サーバーが `hpke-v1` で `content` を暗号化する先の base64url 形式の生 X25519 公開鍵。§暗号化された応答を参照 |
...
| `encryptedContent` | `boolean` | 任意 | §暗号化された応答に従って `content` が暗号化されたかどうか |

サーバーは capability で宣言した `proofFormat` のみを返す。クライアントも宣言した形式のみを検証する。
`proof` または `teeAttestation` を含む結果がいずれかのコミットメントを欠いている場合、検証者はそれを未検証（証明がない場合と同等）として扱わなければならない。
...
| `formats` | `object` | 形式ごとの任意の上書き：`{ "<proofFormat>": { "circuitHash", "verificationKeyUri" } }`。ネゴシエートされた形式について存在する場合、その値がトップレベルの `circuitHash`／`verificationKeyUri` より優先され、トップレベルの値はデフォルトとして機能する。異なる成果物を持つ形式（例：1 つのツールで `snarkjs-v2` と `noir-v1`）を提供するサーバーは `formats` を使用しなければならない |
...
| `encryptedArguments` | `string` | 必須 | base64url 文字列。平文は `{ "salt": "0x...", "arguments": { ... } }` の JCS エンコーディング |
...
ブラインド実行は*入力*を隠すが、それだけでは*出力*に関する情報を隠さない。少数の秘密ビット（`approved`／`declined` など）の関数である出力を持つツールは、そのビットを運営者に漏らす。この形のツールは、（§暗号化された応答を参照）出力をクライアントに暗号化して返すか、運営者が出力を読めない TEE 内で実行すべきである。
...
- すべての形式定義は、基準回路の証明時間とメモリ、証明サイズ、検証時間、検証器の依存フットプリント（npm/WASM か native か）を報告しなければならない。参考実装はこれらを形式ごとに記録し（[`docs/BENCHMARKS.md`](../BENCHMARKS.md) と [`docs/PLAN.md`](../PLAN.md) の Phase 3 行を参照）、`proofFormats` ネゴシエーションがコストを考慮できるようにする。
