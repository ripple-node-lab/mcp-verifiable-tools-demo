...
export function encryptArguments(args: JsonValue, serverBlindPublicKey: string, tool: string): EncryptedArgumentsResult {
  ...
  const aad = new TextEncoder().encode(jcs({ tool, inputCommitment: commitment, encryptionScheme: "hpke-v1" } as JsonValue));
