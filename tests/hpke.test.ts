...
import { createDecipheriv, createPrivateKey, generateKeyPairSync } from "node:crypto";
...
test("HPKE RFC 9180 A.1.1 vector and round trip", () => {
  ...
  const expectedKey = hex("4531685d41d65f03dc48f6b8302c05b0");
  const expectedBaseNonce = hex("56d890e5accaaf011cff4b7d");
  const expectedCiphertext = hex("f938558b5d72f1a23810b4be2ab4f84331acc02fc97babc53a52ae8218a355a96d8770ac83d07bea87e13c512a");
  assert.equal(Buffer.from(expectedKey).toString("hex"), "4531685d41d65f03dc48f6b8302c05b0");
  assert.equal(Buffer.from(expectedBaseNonce).toString("hex"), "56d890e5accaaf011cff4b7d");
  assert.equal(Buffer.from(sealed.slice(32)).toString("hex"), Buffer.from(expectedCiphertext).toString("hex"));
  const direct = createDecipheriv("aes-128-gcm", expectedKey, expectedBaseNonce);
  direct.setAAD(aad);
  direct.setAuthTag(expectedCiphertext.slice(-16));
  assert.equal(Buffer.from(Buffer.concat([direct.update(expectedCiphertext.slice(0, -16)), direct.final()])).toString("utf8"), "Beauty is truth, truth beauty");
