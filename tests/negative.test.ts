...
test("verification key pinning rejects a changed key", async () => {
  ...
  let current = first;
  globalThis.fetch = (async () => ({ ok: true, text: async () => current })) as unknown as typeof fetch;
  try {
    const registry = new VerificationKeyRegistry();
    await registry.get("0xcircuit", "http://one");
    current = second;
    await assert.rejects(() => registry.get("0xcircuit", "http://two"));
  } finally {
